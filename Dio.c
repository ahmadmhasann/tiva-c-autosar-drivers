...
void Dio_WriteChannel(Dio_ChannelType ChannelId, Dio_LevelType Level)
{
	...
	if (FALSE == error)
	{
		/* 	Point to the correct PORT register according to the 
			Port Id stored in the Port_Num member 				*/
...
Dio_LevelType Dio_ReadChannel(Dio_ChannelType ChannelId)
{
	volatile uint32 *Port_Ptr 	= NULL_PTR				;
	boolean 		 error 		= FALSE					;
	Dio_LevelType 	 result 	= (Dio_LevelType)STD_LOW;
	...
	if (FALSE == error)
	{
		/* Point to the correct PORT register according to 
		   the Port Id stored in the Port_Num member 		*/
		switch (Dio_PortChannels[ChannelId].Port_Num)
		{
		case 0:
			Port_Ptr = &GPIO_PORTA_DATA_REG;
 ...
 * Parameters (out): 	None
 * Return value: 		Dio_LevelType
 * Description: 		Function to flip the level of a channel
...
Dio_LevelType Dio_FlipChannel(Dio_ChannelType ChannelId)
{
	...
	volatile uint32 *Port_Direction_Ptr = NULL_PTR				;
	boolean 		 error 				= FALSE					;
	Dio_LevelType 	 status 			= (Dio_LevelType)STD_LOW;
	...
	if (FALSE == error)
	{
		...
		{
		case 0:
			port_data_ptr 		= &GPIO_PORTA_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTA_DIR_REG	;
			break;
		case 1:
			port_data_ptr 		= &GPIO_PORTB_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTB_DIR_REG	;
			break;
		case 2:
			port_data_ptr 		= &GPIO_PORTC_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTC_DIR_REG	;
			break;
		case 3:
			port_data_ptr 		= &GPIO_PORTD_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTD_DIR_REG	;
			break;
		case 4:
			port_data_ptr 		= &GPIO_PORTE_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTE_DIR_REG	;
			break;
		case 5:
			port_data_ptr 		= &GPIO_PORTF_DATA_REG	;
			Port_Direction_Ptr 	= &GPIO_PORTF_DIR_REG	;
			break;
		}
		
		if (BIT_IS_SET(*Port_Direction_Ptr, Dio_PortChannels[ChannelId].Ch_Num))	/* Pin is output */
		{
			if (BIT_IS_SET(*port_data_ptr, Dio_PortChannels[ChannelId].Ch_Num))
			{
				/* Pin is High */
				status = (Dio_LevelType)STD_LOW;
				CLEAR_BIT(*port_data_ptr, Dio_PortChannels[ChannelId].Ch_Num);
			}
			else
			{
				/* Pin is Low */
				status = (Dio_LevelType)STD_HIGH;
